...
function send_message($serial, $instruction, $addrh, $addrl, $data1, $data2, $text)
{
	...
	//print "\n\nInstructie unpacked: ".$instruction." ".$addrh." ".$addrl." ".$data1." ".$data2." ".$sum."\n";
	$message = pack("CCCCCCC", $instruction, $addrh, $addrl, $data1, $data2, $sum, 13);
	//print "\n\nInstructie packed: ".$message."\n\n";
	$try = 0;
	$result = -1;
	do
	{
...
//				print("Unit busy");
			...
			else
			{
				...
				//echo "bitshift result: ".print_r($result)."\n";
			}
		}
...
function read_all_units($db, $serial, $instruction)
{
	...
	$sql = "SELECT $FIELD_UNIT,$FIELD_UNIT_VERSION FROM $TABLE_M_UNITS ORDER BY $FIELD_UNIT";
	if (!($mysql_result = mysql_query($sql))) { print(mysql_error($db) ."\n"); return; }
